...
class WriteInsuranceReviewDialog extends StatefulWidget {
  final HealthInsuranceModel insurance;
  const WriteInsuranceReviewDialog({
    Key key,
    this.insurance,
  }) : super(key: key);

  @override
  _WriteInsuranceReviewDialogState createState() => _WriteInsuranceReviewDialogState();
}

class _WriteInsuranceReviewDialogState extends State<WriteInsuranceReviewDialog> {
  double ratingVal;
          ...
          children: [
            ...
            Row(
              ...
              children: [
                ...
                SizedBox(width: 8.w),
                //post
                BlocConsumer<PostInsuranceReviewCubit, PostInsuranceReviewState>(
                  listener: (context, state) {
                    if (state is PostReviewDone) {
                      context.read<InsuranceReviewsCubit>()..loadInsuranceReviews();
                      Navigator.pop(context);
                    }
                  },
                      ...
                      return TextButton(
                        ...
                        onPressed: () async {
                          if (ratingVal != null && reviewTextVal != null) {
                            context.read<PostInsuranceReviewCubit>()
                              ..postReview(
                                user: 1,
                                rating: ratingVal.toDouble(),
                                comment: reviewTextVal,
                                insurance: widget.insurance.id,
                              );
                          }
                        },
                      );
